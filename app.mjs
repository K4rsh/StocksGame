...
async function getStockQuote(symbol) {
  console.log('Getting stock quote for the symbol: ', symbol);
  try {
    const response = await axios.get(`https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=SXTSD6JIPR50F0FQ`);
    console.log(response.data);
    if (response.data['Global Quote']) {
...
app.get('/stock-price', async (req, res) => {
  try {
    ...
    console.log('Error in /stock-price:', error);
...
app.post('/login', async (req, res) => {
  try {
    ...
    res.send({ token: user.token, isAdmin: user.isAdmin}); //To include isAdmin in the response for seperate pathways for admin and non-admin user players
  } catch (error) {
...
app.get('/profile', async (req, res) => {
  try {
    ...
    // Send the player's portfolio and cash as the response
    res.send({ portfolio: player.portfolio, purse: player.cash });
  } catch (error) {
    res.status(500).send('Internal server error.');
  }
});
...
app.post('/games/:gameId/buy', async (req, res) => {
  ...
  if (shares === undefined) {
    return res.status(400).send('Number of shares not provided.');
  }
  shares = Number(shares);
  if (isNaN(shares) || shares <= 0) {
    return res.status(400).send('Invalid number of shares.');
  }
  ...
  if (!player.portfolio[symbol]) {
    player.portfolio[symbol] = 0; // Initialize the stock in the portfolio if it doesn't exist
  }
...
app.post('/games/:gameId/sell', async (req, res) => {
  ...
  if (!player.portfolio[symbol]) {
    return res.status(400).send('You do not own any shares of this stock.');
  }
